...
import com.example.worldcuppredictor.api.dto.request.MatchListRequest;
...
public class MatchController {
    ...
    @Operation(
            ...
            summary = "Get matches by identifier",
            description = "Returns a top-level JSON array of match objects filtered by identifier.",
            security = {@SecurityRequirement(name = "cookieAuth")}
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Matches loaded successfully", content = @Content(array = @ArraySchema(schema = @Schema(implementation = MatchResponse.class)))),
    ...
    @PostMapping
        public List<MatchResponse> listByIdentifier(@RequestBody MatchListRequest request) {
                ...
                return matchCatalogRepository.findAllByIdentifierIgnoreCaseOrderByMatchDateAscIdAsc(request.getIdentifier().trim()).stream()
                .map(this::toResponse)
                .toList();
    }

    private MatchResponse toResponse(com.example.worldcuppredictor.domain.entity.MatchCatalog match) {
